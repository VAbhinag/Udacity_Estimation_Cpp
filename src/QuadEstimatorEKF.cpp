...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> UpdateIMU = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));

  UpdateIMU.IntegrateBodyRate(gyro,dtIMU);


  float predictedPitch = UpdateIMU.Pitch();
  float predictedRoll = UpdateIMU.Roll();
  ekfState(6) = UpdateIMU.Yaw();	// yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  V3F accel_dot = attitude.Rotate_BtoI(accel);

  predictedState(3) = curState(3) + dt * accel_dot.x;
  predictedState(4) = curState(4) + dt * accel_dot.y;
  predictedState(5) = curState(5) + dt * accel_dot.z - 9.81*dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 1) = -(sin(phi) * sin(theta) * cos(psi)) - (cos(phi) * cos(psi));
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  hPrime(0, 6) = 1;
  zFromX(0) = ekfState(6);
  float correction = z(0) - zFromX(0);
  if (correction > F_PI) {
      zFromX(0) += 2.f * F_PI;
  }
  else if (correction < F_PI) {
      zFromX(0) -= 2.f * F_PI;
  }
